...
@attr(client_min_version=1, client_max_version=base_test.LATEST_API_VERSION)
class AppWithTracingNoIPTestCase(base_test.BaseServerTestCase):
    ...
    def setUp(self):
        super(AppWithTracingNoIPTestCase, self).setUp()
        # Dummy call to invoke the tracer initialization if enabled.
        self.app.get('/')

    def create_configuration(self):
        test_config = super(
            AppWithTracingNoIPTestCase, self).create_configuration()
        test_config.enable_tracing = True
        test_config.tracing_endpoint_ip = None
        return test_config
...
@attr(client_min_version=1, client_max_version=base_test.LATEST_API_VERSION)
class AppWithTracingTestCase(base_test.BaseServerTestCase):
    """Test the basic setup of the tracer and dispatch wrapping for the
    ...
    def setUp(self):
        super(AppWithTracingTestCase, self).setUp()
        # Dummy call to invoke the tracer initialization if enabled.
        self.app.get('/')

    def create_configuration(self):
        test_config = super(
            AppWithTracingTestCase, self).create_configuration()
        test_config.enable_tracing = True
    ...
    def test_does_init_tracer(self):
        self.jaeger_mock_config.assert_called_with(
            config={
                ...
                'local_agent': {
                    'reporting_host': self._tracing_endpoint_ip},
                'logging': True
            },
    ...
    def test_adds_error_to_tags(self):
        ...
        kwargs = {'tags': {"Extract failed": str(e)},
        ...
        self.app.get('/')
        tracer.start_span.assert_called_with(**kwargs)
